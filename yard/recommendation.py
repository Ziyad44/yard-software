...
def build_candidate_actions(state: YardState, config: YardConfig) -> list[Action]:
    ...
    if floor_dock_ids:
        ...
        worker_source = next(
            (
                dock_id
                for dock_id in sorted(floor_dock_ids, key=lambda d: workers[d], reverse=True)
                if dock_id != target_by_floor_pressure and workers[dock_id] > 0
            ),
            None,
        )
        if shifted_workers[target_by_floor_pressure] < config.max_unloaders_per_dock:
            if worker_source is not None:
                shifted_workers[worker_source] -= 1
                shifted_workers[target_by_floor_pressure] += 1
                worker_note = (
                    f"Shift one worker from Dock {worker_source} to Dock {target_by_floor_pressure}."
                )
            elif idle_workers > 0:
                shifted_workers[target_by_floor_pressure] += 1
                worker_note = f"Allocate one idle worker to Dock {target_by_floor_pressure}."
            else:
                worker_note = ""
        else:
            worker_note = ""
    ...
    if pallet_dock_ids:
        ...
        forklift_source = next(
            (
                dock_id
                for dock_id in sorted(pallet_dock_ids, key=lambda d: forklifts[d], reverse=True)
                if dock_id != target_by_pallet_pressure and forklifts[dock_id] > 0
            ),
            None,
        )
        if forklift_source is not None:
            shifted_forklifts[forklift_source] -= 1
            shifted_forklifts[target_by_pallet_pressure] += 1
            forklift_note = (
                f"Shift one forklift from Dock {forklift_source} to Dock {target_by_pallet_pressure}."
            )
        elif idle_forklifts > 0:
            shifted_forklifts[target_by_pallet_pressure] += 1
            forklift_note = f"Allocate one idle forklift to Dock {target_by_pallet_pressure}."
        else:
            forklift_note = ""
    ...
    if target_family == "floor":
        clear_worker_source = next(
            (
                dock_id
                for dock_id in sorted(floor_dock_ids, key=lambda d: workers[d], reverse=True)
                if dock_id != target_by_ratio and workers[dock_id] > 0
            ),
            None,
        )
        ...
        clear_forklift_source = next(
            (
                dock_id
                for dock_id in sorted(pallet_dock_ids, key=lambda d: forklifts[d], reverse=True)
                if dock_id != target_by_ratio and forklifts[dock_id] > 0
            ),
            None,
        )
